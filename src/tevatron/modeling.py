...
from transformers import AutoModel, AutoModelForMaskedLM, PreTrainedModel, AutoTokenizer
...
class SpladePooler(nn.Module):
    ...
    def forward(self, embeddings: Tensor, attention_mask: Tensor, top_k: int, alpha: float = None):
...
class DenseModel(nn.Module):
    ...
    def forward(
            ...
            teacher_query: Dict[str, Tensor] = None,
            teacher_passage: Dict[str, Tensor] = None,
    ):
        ...
        if self.training:
            if self.train_args.negatives_x_device:
                ...
                teacher_query = {"input_tensors": self.dist_gather_tensor(teacher_query["input_tensors"])}
                teacher_passage = {"input_tensors": self.dist_gather_tensor(teacher_passage["input_tensors"])}
            ...
            teacher_scores = torch.matmul(teacher_query["input_tensors"], teacher_passage["input_tensors"].transpose(0, 1))
            teacher_scores = teacher_scores.view(effective_bsz, -1)
            ...
            if self.train_args.negatives_x_device:
               ce_loss = ce_loss * self.world_size  # counter average weight reduction 

            kl_loss = self.kl_loss(scores, teacher_scores)
    ...
    def encode_passage(self, psg):
        ...
        p_reps = self.pooler(p_hidden, psg["attention_mask"], self.top_k, self.alpha)
        return p_reps
    ...
    def encode_query(self, qry):
        ...
        q_reps = self.pooler(q_hidden, qry["attention_mask"], self.top_k, self.alpha)
        return q_reps

    
    def kl_loss(self, feat1, feat2):
        bs, num_p = feat1.shape
        indices = torch.tensor([[x, x+1] for x in range(0, num_p, 2)]).to(feat1.device)
        
        feat1 = feat1.gather(1, indices)
        feat2 = feat2.gather(1, indices)
        feat1 = F.log_softmax(feat1, dim=1)
        feat2 = F.softmax(feat2, dim=1)
        return self.kl(feat1, feat2).sum(dim=1).mean(dim=0)
    ...
    def build(
        ...
        if model_args.decoder_weights:
            lm_q.cls.predictions.decoder = torch.load(model_args.decoder_weights, map_location="cpu")
            lm_p.cls.predictions.decoder = torch.load(model_args.decoder_weights, map_location="cpu")
